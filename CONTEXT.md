...
**Live TV view**:
The screen the addon opens into after startup autoplay: an Icon rail, a permanent Channel panel beside it, the Guide, and a Programme detail strip above it. Replaces the former separate channel list and home menu.
...
**Channel panel**:
The permanent, full-height panel between the Icon rail and the Guide (no slide-out, no dim layer): a header reading the active filter's name (All channels, Favourites, or a Group) with a filter glyph, and rows listing that filter's Channels (number, logo tile, name), the focused row filled blue and the playing row marked by the blue accent, like the Channel column. OK on the header opens the Groups picker in place of the channel rows. OK or Right on a channel row jumps to that Channel's row in the Channel column. Back on the channel rows closes the Live TV view.
_Avoid_: Groups panel, Groups drawer, channel list

**Groups picker**:
The in-panel list of All channels, Favourites and each Provider's Groups under collapsible Provider headers, shown in the Channel panel's rect in place of the channel rows. OK or Right on a row applies that filter, updating the header, the panel rows and the Guide together, and returns to the channel rows with focus staying in the panel. OK on a Provider header toggles its collapse; Right on a Provider header, or Back, returns to the channel rows without changing the filter.
_Avoid_: group picker, Groups pane, channels mode
...
**Remote-hint bar**:
The bar along the bottom of the Live TV view listing what OK, Back, Left/Right, Info and long-press do in the current focus zone; shown only while browsing, hidden while Playback is up. Reads "OK Channels" in the Channel panel's channels mode, "OK Groups" in the Groups picker, and the Channel column's second hint reads "← Groups".
