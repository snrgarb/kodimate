...
**Live TV view**:
The screen the addon opens into after startup autoplay: an Icon rail, a permanent Groups panel beside it, the Guide, and a Programme detail strip above it. Replaces the former separate channel list and home menu.
...
**Groups panel**:
The permanent full-height panel between the Icon rail and the Guide: a static "Groups" header with the filter glyph (never focusable), rows for All groups, Favourites, and each enabled Provider's Groups under collapsible Provider headers; OK on a row applies that filter to the Guide (Channel column, grid, Programme detail strip) at once and keeps focus in the panel; Right on a row applies it and moves into the Channel column; OK on a Provider header toggles collapse; Right on a Provider header moves into the Channel column without changing the filter; the active filter's row carries the blue accent; focused row filled blue; Back closes the Live TV view.
_Avoid_: Channel panel, Groups picker, Groups drawer, channels mode
...
**Channel column**:
The Guide's list of Channels, one per row with number and a logo tile (placeholder when the Channel has no logo); the name lives in the Programme detail strip; together with the programme cells it forms the Guide grid.
...
**Remote-hint bar**:
The bar along the bottom of the Live TV view listing what OK, Back, Left/Right, Info and long-press do in the current focus zone; shown only while browsing, hidden while Playback is up. Reads "OK Select" and "→ Channels" in the Groups panel; the Channel column's second hint reads "← Groups".
