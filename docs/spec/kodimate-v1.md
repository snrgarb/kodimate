...
24. As a Kodi user, I want an open Live TV view to update in place when a refresh completes, so that I don't have to back out and reopen the screen to see new channels.
...
29. As a Kodi user, I want the addon to open straight into the Live TV view after startup autoplay, so that I see channels and what's on without passing through a menu.
30. As a Kodi user, I want the Guide grid to be my channel list, so that I never have to choose between "channels" and "guide".
31. As a Kodi user, I want a thin icon rail with Live TV, Catch-up and Settings always visible at the edge of the screen, so that the other screens are one or two presses away without a home menu.
32. As a Kodi user, I want channels grouped into the provider's own categories in the Groups panel, so that I can browse by genre or bundle the way my provider organised them.
33. As a Kodi user, I want Left from the channel column to slide out the Groups panel, listing each provider's groups under a collapsible provider header with a flat All channels and Favourites at the top, so that changing what I'm looking at is one press away.
34. As a Kodi user, I want OK or Right on a group in the panel to apply it, close the panel, and return focus to the channel column, so that picking a group is a single gesture.
35. As a Kodi user, I want the panel to leave out groups with no listable channels and highlight the currently applied group next time it opens, so that I never pick an empty group and always know what I filtered by.
36. As a Kodi user, I want the current filter name shown above the channel column when the panel is closed, so that I know which group I'm looking at.
37. As a Kodi user, I want Back to close the Groups panel if it's open, and otherwise close the Live TV view, so that Back always peels exactly one layer.
38. As a Kodi user, I want channels numbered from the provider's own numbering (or provider order if it has none), so that the list matches numbers I'm used to from other apps.
39. As a Kodi user, I want to renumber a channel myself, so that I can put my favourite channels at low numbers.
40. As a Kodi user, I want to hide a channel I never watch, so that it stops cluttering my list.
41. As a Kodi user, I want a "show hidden" toggle at the foot of the Groups panel, so that I can still find and unhide a channel I hid by mistake.
42. As a Kodi user, I want to mark a channel as a favourite, so that it shows up in one flat Favourites list regardless of its provider or group.
43. As a Kodi user, I want to reorder my Favourites list by hand, so that my most-watched channels sit at the top.
44. As a Kodi user, I want to reset a channel's overrides back to what the provider supplied, so that I can undo my own customisations without deleting the channel.
45. As a Kodi user, I want all of these edits reachable from a single context menu on a focused channel row on the Live TV view, so that I don't have to learn a separate editing mode.
46. As a Kodi user, I want a channel that disappears from a provider's list to stay in place with its overrides intact for a week before being purged, so that a transient playlist error doesn't wipe out my customisations.
47. As a Kodi user, I want the same channel offered by two different providers to appear as two separate rows, so that each provider's own numbering, archive support, and reliability stay visible to me.
48. As a Kodi user, I want a channel's logo shown next to its name, sourced straight from the provider, so that I can recognise channels visually.
49. As a Kodi user, I want a channel with a broken or missing logo to fall back to a bundled placeholder image, so that the list never shows a broken image icon.
50. As a Kodi user, I want a Now strip above the grid showing the current date/time and the focused channel's current programme with progress, so that I can read what's on without opening a dialog.
51. As a Kodi user, I want the Now strip to follow the focused row as I move Up/Down, so that it always describes the channel I'm on.
52. As a Kodi user, I want the Now strip to say "No information" when the focused channel has no programme now, so that it never goes blank.

### EPG / Guide

53. As a Kodi user, I want a guide grid showing channels down the side and a scrolling timeline of programmes across, so that I can see what's on now and later across all my channels at once.
54. As a Kodi user, I want a now-line marking the current time on the guide, so that I can tell at a glance what's live.
55. As a Kodi user, I want to move the guide cursor left/right within a channel's row to select adjacent programmes, and up/down to move to the next channel at the same time position, so that browsing feels natural on a D-pad.
56. As a Kodi user, I want programme cells sized proportionally to their duration, so that a 30-minute show and a 3-hour film are visually distinguishable.
57. As a Kodi user, I want a channel with no EPG data to show clearly as having no programme information rather than an empty or broken cell, so that I'm not confused about whether something is wrong.
58. As a Kodi user, I want OK on a past programme cell to show me its details and, if catch-up is available, let me play it, so that browsing the guide is how I discover catch-up content.
59. As a Kodi user, I want OK on the currently-airing cell to offer both "watch live" and "start over from the beginning", so that I can choose either.
60. As a developer, I want the guide grid built from a control pool of reusable programme cells rather than adding/removing controls per move, so that scrolling and cursor movement stay smooth on real hardware.

### Playback OSD and zapping

61. As a Kodi user, I want the last channel I was watching to autoplay when I start the addon, so that I land straight back in live TV.
62. As a Kodi user, I want the channel list to optionally overlay on top of autoplay, so that I can immediately switch away if autoplay picked the wrong channel.
63. As a Kodi user, I want Up/Down or OK on bare video to open a channel list without changing the stream, so that browsing channels never zaps by accident.
64. As a Kodi user, I want the channel list split into a Groups pane and a Channels pane side by side, so that I can narrow down by group before picking a channel.
65. As a Kodi user, I want OK on a channel in that list to actually zap to it and close the list, so that confirming a choice is a single deliberate action.
66. As a Kodi user, I want to type a channel number directly during playback and have it zap once I stop typing or press OK, so that I can jump straight to a known channel number.
67. As a Kodi user, I want an on-screen info bar showing the channel number, logo, name, current programme with progress, and the next programme, so that I always know what I'm watching and what's coming up.
68. As a Kodi user, I want that info bar to auto-hide after a few seconds and be summonable again with OK, so that it doesn't stay in my way.
69. As a Kodi user, I want the screen to go black with a spinner and immediately show the new channel's name while it's connecting, so that zapping feels responsive even during a slow provider handshake.
70. As a Kodi user, I want Back during playback to close whatever overlay is open first, and only leave playback once nothing is open, so that Back behaves predictably layer by layer.
71. As a developer, I want a single owned `xbmc.Player` instance for the whole addon, so that no stale player instance keeps a provider connection open after I've moved away from a channel.

### Stream failure handling

72. As a Kodi user, I want the app to automatically retry a channel a few times if the stream drops mid-play, so that a brief network blip doesn't force me to zap the channel myself.
73. As a Kodi user, I want the app to try an alternate stream format automatically if the first one is rejected on an Xtream provider, so that a provider that only serves `.m3u8` (or only `.ts`) still works without me configuring anything.
74. As a Kodi user, I want the app to remember which stream format worked for a provider, so that it doesn't have to rediscover it on every channel.
75. As a Kodi user, I want a clear message telling me whether a channel failed because my login was rejected, because I've hit my provider's connection limit, or because the stream just wasn't available, so that I know whether the problem is on my end or the provider's.
76. As a Kodi user, I want the channel list to open automatically when a channel fails for good, so that I can immediately try something else.
77. As a Kodi user, I want to retry a failed channel from scratch with OK, so that I can try again after a provider issue clears up.
78. As a Kodi user, I want any zap, number entry, or Back press to immediately cancel a channel that's still trying to connect, so that the app never fights me for control while it's retrying.
79. As a developer, I want every playback attempt logged at INFO with channel, provider, attempt number, format, and outcome, with credentials redacted, so that stream failures are diagnosable from `kodi.log` without exposing secrets.

### Catch-up

80. As a Kodi user, I want to open a past programme cell in the guide and play it from the beginning, so that I can catch up on something I missed.
81. As a Kodi user, I want a dedicated Catch-up browser listing channels that support it, with their past programmes grouped by day, so that I can browse catch-up content without going through the guide.
82. As a Kodi user, I want Left/Right on the playback OSD to step to the previous or next programme on the current channel, playing past ones via catch-up, so that I can move through a channel's schedule without leaving playback.
83. As a Kodi user, I want a single consistent "Programme info" dialog no matter which of these three ways I got there, so that the catch-up experience feels like one feature.
84. As a Kodi user, I want to start the currently-airing programme over from the beginning, so that I can watch something I tuned into partway through.
85. As a Kodi user, I want the guide to visually mark past programmes I can actually play versus ones I can't, so that I don't try to catch up on something outside my provider's archive window.
86. As a Kodi user, I want the next programme to auto-play after a short countdown when a catch-up programme ends, so that I can binge through a channel's schedule without extra button presses.
87. As a Kodi user, I want to cancel that auto-play countdown and go back to where I came from, so that I'm not forced into something I didn't ask for.
88. As a Kodi user, I want catch-up start times corrected for my provider's timezone, so that "8pm" in the guide actually plays the 8pm programme.
89. As a Kodi user, I want to zap away from a catch-up programme to a live channel at any time, exactly like normal zapping, so that catch-up doesn't feel like a separate, more restrictive mode.
90. As a Kodi user, I want a clear message and a return to where I came from if a catch-up programme's stream fails to start, so that I'm not stuck staring at a black screen.
91. As a developer, I want catch-up URLs built per the exact convention my provider uses (Xtream timeshift path/query forms, or one of the M3U catchup modes), so that catch-up works across the range of real-world providers.

### Search

92. As a Kodi user, I want to search across both channel names and programme titles, so that I can find something without knowing which channel it's on.
93. As a Kodi user, I want search results split into a Channels section (OK plays) and a Programmes section (OK opens the Programme info dialog), so that the two kinds of result behave the way I'd expect.
94. As a Kodi user, I want search to ignore case and match partial text, so that I don't need to type an exact channel or programme title.
95. As a Kodi user, I want search to exclude hidden and stale channels and programmes on channels I can't otherwise see, so that results only ever point at things I could actually watch anyway.

### Settings and globals

96. As a Kodi user, I want to control the refresh interval, whether refresh runs at startup, autoplay behaviour, OSD auto-hide timing, and number-entry commit delay from Kodi's own settings screen, so that I can tune the addon without a dedicated screen for every knob.
97. As a Kodi user, I want a "Manage providers" button in Kodi's settings that jumps straight to the Providers screen, so that provider management is reachable from the place I'd naturally look for it.
98. As a Kodi user, I want a debug logging toggle, so that I can turn on verbose logs only when I need to report a problem.

### Packaging, i18n, and dev workflow

99. As a developer, I want every user-facing string sourced from `strings.po` ids from day one, so that translation is never a retrofit.
100. As a Kodi user, I want the addon shipped in British English, so that spelling matches the rest of Kodi's own UI on my system.
101. As a developer, I want a single 1080i skin that Kodi scales to other resolutions, so that I don't maintain multiple skin resolutions in v1.
102. As a developer, I want the addon's version tracked in `addon.xml` using semantic versioning, so that installs and updates are unambiguous.
103. As a developer, I want a deploy script that builds an installable zip, so that I can produce a release artefact without manual packaging steps.
104. As a developer, I want tagged GitHub releases carrying that zip as an asset, so that users have a stable place to download a given version.
105. As a developer, I want a symlink-based local dev loop against a real Kodi install, so that I can iterate on the addon without repackaging on every change.
106. As a developer, I want a log-tailing helper, so that I can watch `kodi.log` while exercising the addon live.
...
On a Generation change, an open Live TV view re-queries and re-renders in place, keeping its filter, its focused Channel (by Channel Key, falling to the nearest row if that channel went Stale), and its scroll offset; the grid re-renders only its visible viewport. Re-render is deferred while any modal (Programme info dialog, context menu, number entry) is open and applied once it closes; if the filtered Group or Provider vanished entirely, the filter falls back to "All channels" (or stays on Favourites, if that was active) with no toast, and focus falls to the first listable row. The OSD's now/next display re-reads programmes by Channel Key on a Generation change without touching the running stream.
...
Playback behaviour: seeking steps natively inside the player's own buffer (Kodi's `videoplayer.seeksteps`/`seekdelay` settings) and, once a seek or a pause-then-resume target falls outside that buffer, rebuilds the stream as a Catch-up URL at the target position — no seek offered at all if the stream reports no duration and no Catch-up Window is available. Pausing beyond a live stream's buffer likewise resumes via a rebuilt Catch-up URL at the elapsed position rather than failing; a Catch-up session's own buffer is a fixed range, so its pause/resume is always a native player toggle. At the end of a catch-up programme, a 5-second "Up next" countdown plays the next Programme on the same channel; the player is always stopped and `onPlayBackStopped` awaited before the next `play()` (important for single-connection accounts); if the next Programme is airing now, the live URL is used instead and the catch-up badge drops. Back during the countdown cancels it and returns to the Origin screen. The OSD during Catch-up shows the same bar plus a CATCH-UP badge, with position/duration replacing wall-clock progress; Up/Down still opens the channel list, and OK on any channel there leaves Catch-up for live playback of that channel (as does number entry). Back returns to the Origin screen — the screen the Playback Session was started from (Live TV view or Catch-up browser). There is no resume position in v1.
...
- A local timeshift proxy for live pause/buffering (ADR 0006 explicitly avoids one); pausing/seeking beyond the player's own buffer instead rebuilds a Catch-up URL, per "Playback behaviour" above.
...
- Catch-up extras: per-programme resume position, and an arbitrary-time picker for archive content on channels without matching EPG data.
