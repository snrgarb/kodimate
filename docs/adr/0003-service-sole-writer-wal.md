...
- The service is the only process that writes `channel`, `channel_group`, `programme`, `epg_source`, and `provider.last_refresh_at`/`last_error`/`account_expires_at`/`max_connections`/`allowed_output_formats`/`server_timezone`. The script writes only `channel_override`, `provider` CRUD/config columns, `provider.learned_stream_format`, `provider.catchup_url_form`, and `meta.last_channel_provider_id`/`meta.last_channel_key`. A manual Refresh from the UI is a request to the service, never run inline in the script.
